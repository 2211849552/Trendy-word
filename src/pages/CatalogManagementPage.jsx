...
export function CatalogManagementPage() {
  ...
  return (
    <>
      ...
      <section
        className="mt-8 rounded-2xl bg-brand-200 p-6 shadow-premium ring-1 ring-slate-100/80"
        dir="rtl"
      >
          ...
          <div className="mt-6 space-y-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative min-w-0 flex-1 sm:max-w-md">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/10 py-2.5 pe-10 ps-3 text-sm outline-none ring-[#0056D2]/20 focus:border-[#0056D2] focus:ring-2"
                />
              </div>
              <button
                type="button"
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
              {filteredCategories.map((cat) => (
                <article
                  key={cat.id}
                  className="flex flex-col rounded-2xl border border-white/5 bg-brand-200 p-5 text-center shadow-premium ring-1 ring-slate-50"
                >
                  ...
                  <div className="mt-3 flex flex-wrap justify-center gap-1">
                    {(cat.sizes ?? CLOTHING_SIZES).map((s) => (
                      <span
                        key={s}
                        className="rounded-md bg-brand-300 px-2 py-0.5 text-[10px] font-bold tabular-nums text-white/70 ring-1 ring-slate-200/80"
                      >
                        {s}
                      </span>
                    ))}
                  ...
                  <p className="text-xs text-white/60">منتج</p>
            ...
            {filteredCategories.length === 0 ? (
              <p className="py-10 text-center text-sm text-white/60">لا توجد تصنيفات مطابقة للبحث.</p>
            ) : null}
          </div>
        ) : (
          <div className="mt-6 space-y-4">
              ...
              <table className="w-full min-w-[720px] border-collapse text-sm">
                <thead>
                  <tr className="border-b border-white/10 bg-brand-300/80 text-start">
                    ...
                    <th className="px-4 py-3 font-semibold text-white/80">النوع</th>
                    <th className="px-4 py-3 font-semibold text-white/80">مطلوبة</th>
                    <th className="px-4 py-3 font-semibold text-white/80">التصنيفات</th>
                    <th className="px-4 py-3 font-semibold text-white/80">الإجراءات</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredProperties.map((row) => (
                    <tr key={row.id} className="border-b border-white/5 last:border-0">
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex rounded-full bg-brand-100 px-2.5 py-0.5 text-xs font-semibold text-white/90 ring-1 ring-brand-200">
                      ...
                      <td className="px-4 py-3">
                        {row.required ? (
                          ...
                          <span className="text-white/60">لا</span>
                        )}
                      </td>
                      <td className="px-4 py-3">
            ...
            {filteredProperties.length === 0 ? (
              <p className="py-10 text-center text-sm text-white/60">لا توجد خصائص مطابقة للبحث.</p>
            ) : null}
          </div>
        )}
      </section>
