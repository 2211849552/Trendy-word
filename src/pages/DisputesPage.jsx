    ...
    <div className="mx-auto max-w-6xl space-y-6 pb-20 animate-in fade-in duration-500 relative">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-white/10 bg-brand-200 p-5 shadow-premium text-center flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-2xl font-bold text-white">24 ساعة</p>
        </div>
        ...
        <div className="rounded-xl border border-white/10 bg-brand-200 p-5 shadow-premium text-center flex flex-col items-center justify-center">
          ...
          <p className="mt-1 text-2xl font-bold text-white">{disputes.filter(d => d.status === 'مفتوحة').length}</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {disputes
          ...
          .map(dispute => (
          <div key={dispute.id} className="rounded-xl border border-white/10 bg-brand-200 p-5 shadow-premium transition-shadow hover:shadow-premium">
            ...
            <div className="flex flex-wrap md:flex-nowrap items-start justify-between gap-4 mb-4">
              ...
              <div className="flex-1 order-2 md:order-1 text-right w-full">
                <div className="flex justify-between items-start mb-2">
                   <h3 className="text-xl font-bold text-white flex items-center gap-2 flex-row-reverse">
                     شكوى <span className="text-white/80">{dispute.id}</span>
                   </h3>
                </div>
              ...
              <div className="flex flex-col items-start gap-3 w-full md:w-auto order-1 md:order-2">
                 <div className="flex items-center gap-2 flex-wrap">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                      dispute.type === 'استرجاع' ? 'bg-brand-300 text-brand-700' :
                      dispute.type === 'نزاع' ? 'bg-yellow-100 text-yellow-700' : 'bg-brand-300 text-white/80'
                    }`}>
                      {dispute.type}
                    </span>
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                      dispute.status === 'مفتوحة' ? 'bg-red-100 text-red-700' :
                      dispute.status === 'قيد المراجعة' ? 'bg-yellow-100 text-yellow-700' : 'bg-brand-300 text-white/80'
                    }`}>
                      {dispute.status}
                    </span>
                    {dispute.hasImage && (
            ...
            {dispute.hasImage && (
              ...
              >
                 <div className="flex items-center gap-4">
                    <div className="size-16 rounded-lg bg-brand-200 border border-white/10 shadow-premium overflow-hidden flex items-center justify-center text-3xl">
          ...
          <div className="w-full max-w-4xl rounded-2xl bg-brand-200 shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 my-auto">
            ...
            <div className="p-6 space-y-6">
               ...
               <div className="flex justify-between items-center text-right border-b border-white/5 pb-4" dir="rtl">
                  <div className="flex gap-2">
                     <span className={`px-4 py-1.5 rounded-full text-sm font-bold ${
                      selectedDispute.type === 'استرجاع' ? 'bg-brand-300 text-brand-700' :
                      selectedDispute.type === 'نزاع' ? 'bg-yellow-100 text-yellow-700' : 'bg-brand-300 text-white/80'
                    }`}>
                      {selectedDispute.type}
                    </span>
                    <span className={`px-4 py-1.5 rounded-full text-sm font-bold ${
                      selectedDispute.status === 'مفتوحة' ? 'bg-red-100 text-red-700' :
                      selectedDispute.status === 'قيد المراجعة' ? 'bg-yellow-100 text-yellow-700' : 'bg-brand-300 text-white/80'
                    }`}>
                      {selectedDispute.status}
                    </span>
                  </div>
                  <div className="text-right">
                     <h3 className="text-xl font-bold text-white flex items-center justify-end gap-1">شكوى {selectedDispute.id}</h3>
               ...
               <div className="rounded-xl bg-brand-300 border border-white/5 p-5 text-right" dir="rtl">
                  ...
                  <p className="font-bold text-white text-lg">{selectedDispute.subject}</p>
               </div>
                ...
                {selectedDispute.hasImage && (
                  <div className="rounded-xl border border-brand-200 bg-brand-200 overflow-hidden text-right" dir="rtl">
                     ...
                     <div className="p-6 flex flex-col items-center justify-center">
                        ...
                        <button 
                          onClick={() => setShowImageModal(true)}
                          className="text-white text-sm font-bold mt-2 flex items-center gap-1 hover:underline"
                        >
                           اضغط للتكبير <Search className="size-3" />
                        </button>
                     </div>
                    <div className="p-3 bg-brand-300 text-center border-t border-white/5">
                       <p className="text-xs text-white/60">ملاحظة: الصورة مرفقة من الزبون كدليل على الشكوى</p>
                    </div>
                  </div>
               )}
