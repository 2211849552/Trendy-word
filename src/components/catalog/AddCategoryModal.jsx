...
/**
 * @param {{ open: boolean; onClose: () => void; onSubmit: (payload: { name: string; emoji: string; sizes: string[] }) => void }} props
 */
export function AddCategoryModal({ open, onClose, onSubmit }) {
  const [name, setName] = useState('')
  const [emoji, setEmoji] = useState(CLOTHING_ICON_OPTIONS[0])
  const [sizes, setSizes] = useState(() => new Set(CLOTHING_SIZES))
  ...
  const handleSubmit = (e) => {
    ...
    onSubmit({ name: trimmed, emoji, sizes: ordered })
    onClose()
  }
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
      ...
      >
        <div className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-white/5 bg-brand-200 px-5 py-4">
          <h2 id="add-category-title" className="text-lg font-bold text-white">
            إضافة تصنيف جديد
          </h2>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-5 px-5 py-6">
          <div>
            <label htmlFor="cat-name" className="mb-1.5 block text-sm font-semibold text-white/90">
          ...
          <div>
            ...
            <p className="mb-2 text-xs text-white/60">S، M، L، XL — اختر ما ينطبق على هذا التصنيف</p>
