...
export function DriverSettleDuesModal({
  open,
  driver,
  dueBalance,
  onClose,
  ...
  const balance = Number(dueBalance ?? 0)
  ...
  const validate = () => {
    ...
    if (num > balance) return `المبلغ يتجاوز رصيد المستحقات (${formatDriverCustodyAmount(balance)}).`
    return ''
  }
  ...
  const overlay = (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6">
      ...
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="driver-settle-dues-title"
      ...
      >
        <div className="flex items-center justify-between border-b border-white/5 bg-brand-300/50 px-5 py-4">
          <h2 id="driver-settle-dues-title" className="text-lg font-bold text-white">
            تسوية مستحقات السائق
          </h2>
          <button
            type="button"
            onClick={() => !saving && onClose?.()}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-6">
          <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-center">
            <div className="mx-auto mb-2 flex size-10 items-center justify-center rounded-xl bg-emerald-500/15 text-emerald-300">
              <DollarSign className="size-5" />
            </div>
            <p className="text-xs text-white/60">رصيد المستحقات القابل للتسوية — {driver.name}</p>
            <p className="mt-1 text-xl font-bold text-emerald-300 tabular-nums" dir="ltr">
              {formatDriverCustodyAmount(balance)}
            </p>
          </div>

          <div>
            <label htmlFor="settle-dues-amount" className="mb-1.5 block text-sm font-semibold text-white/90">
              مبلغ التسوية (د.ل) <span className="text-rose-600">*</span>
            </label>
            <input
              id="settle-dues-amount"
              type="number"
              min={0.01}
              max={balance}
              step={0.01}
          ...
          <div>
            <label htmlFor="settle-dues-description" className="mb-1.5 block text-sm font-semibold text-white/90">
              ملاحظة (اختياري)
            </label>
            <textarea
              id="settle-dues-description"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              rows={3}
              placeholder="مثال: تحويل بنكي أو تسليم نقدي"
